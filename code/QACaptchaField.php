...
class QACaptchaField extends SpamProtectorField {
	...
	public function getFormName() {
		if ($this->form) return $this->form->FormName();
	}
	...
	public function validate($validator) {
		...
		try {
			...
			if (!($question && $question->exists()) || !$answer) throw new Exception('Validation failed');

			$correct = $question->checkAnswer($answer);
			if (!$correct) throw new Exception('Validation failed');
		}
		catch (Exception $e) {
